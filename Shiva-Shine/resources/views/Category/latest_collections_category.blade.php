...
<section class="py-14 bg-[#fffaf7]">
    <div class="max-w-7xl mx-auto px-4">
        ...
        <!-- Products Grid -->
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6" id="productGrid">
            @php
                $products = [
            ...
            @foreach ($products as $product)
                <div data-category="{{ $product['category'] }}"
                    class="bg-white rounded-2xl overflow-hidden shadow-sm relative group transition transform hover:-translate-y-1">
...
<script>
    function filterProducts() {
        ...
        cards.forEach(card => {
            ...
            card.classList.toggle('hidden', selectedCategory !== 'all' && cardCategory !== selectedCategory);
        });
    }
</script>
@endsection
